...
import { Conversation, User, Message, Post } from '../../types.ts';
...
const ChatPage: React.FC<ChatPageProps> = ({ currentUser, allUsers, chukUser, initialTargetUser, onViewProfile }) => {
  ...
  useEffect(() => {
    ...
    const handleNewMessage = (message: Message) => {
        ...
        setConversations(prev => {
            ...
            if (convoIndex > -1) {
                const updatedConvo = { ...prev[convoIndex], lastMessage: message.imageUrl ? 'Sent an image' : message.text, timestamp: message.timestamp };
                const restConvos = [...prev.slice(0, convoIndex), ...prev.slice(convoIndex + 1)];
                return [updatedConvo, ...restConvos];
            } else {
                const newConvo = { id: `conv-${otherUserId}`, participants: [{ id: currentUser.id }, { id: otherUserId }], lastMessage: message.imageUrl ? 'Sent an image' : message.text, timestamp: message.timestamp, messages: [] };
  ...
  const handleSendMessage = async (text: string, imageUrl?: string) => {
    ...
    const tempMessage: Message = { id: `temp-${Date.now()}`, senderId: currentUser.id, receiverId: otherUserId, text, imageUrl, timestamp: new Date().toISOString() };
