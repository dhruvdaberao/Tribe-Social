...
import { Post, User, Tribe, Story, NavItem } from '../../types';
...
export const ProfilePage: React.FC<ProfilePageProps> = (props) => {
  ...
  const isBlockedByYou = (currentUser.blockedUsers || []).includes(user.id);
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start">
                ...
                <div className="w-full sm:w-auto pt-2 sm:pt-4 flex items-center space-x-2">
                    {isOwnProfile ? (
                        ...
                        <>
                           <button onClick={() => onStartConversation(user)} className="w-full sm:w-auto font-semibold px-6 py-2 rounded-lg transition-colors bg-surface text-primary border border-border hover:bg-background">Message</button>
                               ...
                               <button onClick={() => setOptionsOpen(!optionsOpen)} onBlur={() => setTimeout(() => setOptionsOpen(false), 150)} className="p-2 rounded-full bg-surface text-primary border border-border hover:bg-background" aria-label="More options"><OptionsIcon /></button>
                                {optionsOpen && (
                                     <div className="absolute right-0 mt-2 w-48 bg-surface rounded-lg shadow-lg border border-border z-10">
                                         ...
                                         <button onClick={handleBlockClick} className={`w-full text-left px-4 py-2 hover:bg-background transition-colors flex items-center space-x-2 rounded-b-lg ${isBlockedByYou ? 'text-green-600' : 'text-red-500'}`}>
                                            <BlockIcon /><span>{isBlockedByYou ? 'Unblock User' : 'Block User'}</span>
                                         </button>
                                     </div>
                                )}
                           </div>
