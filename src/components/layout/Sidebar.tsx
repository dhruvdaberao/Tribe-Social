...
const NavBadge: React.FC<{ count: number }> = ({ count }) => {
    ...
    return (
        <span className="absolute top-1 right-1 md:top-0 md:right-0 flex items-center justify-center h-4 min-w-[1rem] rounded-full bg-red-500 px-1 text-[10px] font-bold text-white ring-2 ring-surface transform md:translate-x-1/4 md:-translate-y-1/4">
            {count > 99 ? '99+' : count}
        </span>
    );
};
