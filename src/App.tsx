...
// // Components
// import Sidebar from './components-prev/layout/Sidebar';
// import FeedPage from './components-prev/feed/FeedPage';
// import { ProfilePage } from './components-prev/profile/ProfilePage';
// import ChatPage from './components-prev/chat/ChatPage';
// import DiscoverPage from './components-prev/discover/DiscoverPage';
// import LoginPage from './components-prev/auth/LoginPage';
// import TribesPage from './components-prev/tribes/TribesPage';
// import TribeDetailPage from './components-prev/tribes/TribeDetailPage';
// import EditTribeModal from './components-prev/tribes/EditTribeModal';
// import TribeMembersModal from './components-prev/tribes/TribeMembersModal';
// import CreatePost from './components-prev/feed/CreatePost';
// import NotificationsPage from './components-prev/notifications/NotificationsPage';
// import SettingsPage from './components-prev/settings/SettingsPage';
// import StoryCreator from './components-prev/stories/StoryCreator';
// import StoryViewer from './components-prev/stories/StoryViewer';
// import StoryFeed from './components-prev/stories/StoryFeed';
// import { Toaster, toast } from './components-prev/common/Toast';
...
// const PageContainer: React.FC<{ name: NavItem, activeName: NavItem, children: React.ReactNode }> = ({ name, activeName, children }) => (
//     <div style={{ display: name === activeName ? 'block' : 'none' }}>
//         {children}
//     </div>
// );
...
//     const [isFetchingMoreDiscover, setIsFetchingMoreDiscover] = useState(false);
...
//     const [isFetchingTribeMessages, setIsFetchingTribeMessages] = useState(false);
...
//         const author = userMap.get(postFromApi.user) || postFromApi.user; // Use from map or from post itself
...
//             ...postFromApi,
//             author,
//             comments: postFromApi.comments ? postFromApi.comments.map((comment: any) => {
//                 const commentAuthor = userMap.get(comment.user) || comment.user;
...
//                 api.fetchUsers(),
//                 api.fetchFeedPosts(1),
//                 api.fetchPosts(1), // Also fetch initial discover posts
//                 api.fetchTribes(),
...
//             const [usersResult, feedPostsResult, discoverPostsResult, tribesResult, notificationsResult, myStoriesResult, followingStoriesResult] = results;
...
//             const discoverPostsData = discoverPostsResult.status === 'fulfilled' ? discoverPostsResult.value.data : [];
...
//             // Merge feed and discover posts for a richer initial state, avoiding duplicates
//             const combinedPosts = [...feedPostsData, ...discoverPostsData];
//             const uniquePosts = Array.from(new Map(combinedPosts.map(p => [p.id, p])).values());
//             const populatedPosts = uniquePosts.map(populatePost).filter(Boolean);

//             setPosts(populatedPosts as Post[]);
//             setHasMoreFeed(feedPostsData.length > 0);
//             setHasMoreDiscover(discoverPostsData.length > 0);
//             setFeedPage(1);
//             setDiscoverPage(1);
            
//             setTribes(tribesData.map((tribe: any) => ({ ...tribe, messages: [] })));
...
//         if (isFetchingMore || !hasMoreFeed) return;
//         setIsFetchingMore(true);
//         try {
//             const nextPage = feedPage + 1;
//             const { data } = await api.fetchFeedPosts(nextPage);
//             if (data.length > 0) {
//                 const newPosts = data.map(populatePost).filter(Boolean);
...
//         }
//     }, [isFetchingMore, hasMoreFeed, feedPage, populatePost]);
    
//     const loadMoreDiscover = useCallback(async () => {
//         if (isFetchingMoreDiscover || !hasMoreDiscover) return;
//         setIsFetchingMoreDiscover(true);
//         try {
//             const nextPage = discoverPage + 1;
//             const { data } = await api.fetchPosts(nextPage);
//             if (data.length > 0) {
//                 const populated = data.map((p: any) => populatePost(p)).filter(Boolean) as Post[];
//                 // avoid duplicates
//                 const existingPostIds = new Set(posts.map(p => p.id));
//                 const newPosts = populated.filter(p => !existingPostIds.has(p.id));
//                 setPosts(prev => [...prev, ...newPosts].sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime()));
...
//         } finally {
//             setIsFetchingMoreDiscover(false);
//         }
//     }, [isFetchingMoreDiscover, hasMoreDiscover, discoverPage, populatePost, posts]);
...
//             if (populated) {
//                 setPosts(prev => {
//                     // Avoid duplicates from optimistic updates or socket echoes
//                     if (prev.some(p => p.id === populated.id)) return prev;
//                     return [populated, ...prev].sort((a: Post, b: Post) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime())
//                 });
//             }
//         };
...
//         try {
//             // The socket listener will handle adding the post to the state
...
//             toast.error("Could not create tribe.");
//         }
//     };
...
//             setIsFetchingTribeMessages(true);
...
//             toast.error("Could not load tribe messages.");
//         } finally {
//             setIsFetchingTribeMessages(false);
//         }
//     };
...
//           toast.success("Tribe details saved!");
//       } catch (error) {
...
//         return <div className="min-h-screen bg-background flex flex-col items-center justify-center"><img src="/duckload.gif" alt="Loading..." className="w-24 h-24" /><h1 className="mt-4 text-xl font-semibold text-primary">Connecting to Tribe...</h1></div>;
//     }
...
//     const userPosts = viewedUser ? visiblePosts.filter(p => p.author.id === viewedUser.id) : [];
...
//                 <main ref={mainContentRef} className="pt-16 pb-16 md:pb-0 h-screen overflow-y-auto">
//                     <div className={containerClass}>
//                         <PageContainer name="Home" activeName={activeNavItem}>
...
//                             <FeedPage posts={feedPosts} currentUser={currentUser} allUsers={visibleUsers} allTribes={tribes} onLikePost={handleLikePost} onCommentPost={handleCommentPost} onDeletePost={handleDeletePost} onDeleteComment={handleDeleteComment} onViewProfile={handleViewProfile} onSharePost={handleSharePost} onLoadMore={loadMoreFeed} hasMore={hasMoreFeed} isFetchingMore={isFetchingMore} />
//                         </PageContainer>
//                         <PageContainer name="Discover" activeName={activeNavItem}>
//                             <DiscoverPage posts={visiblePosts} users={visibleUsers} tribes={tribes} currentUser={currentUser} onLikePost={handleLikePost} onCommentPost={handleCommentPost} onDeletePost={handleDeletePost} onDeleteComment={handleDeleteComment} onToggleFollow={handleToggleFollow} onViewProfile={handleViewProfile} onViewTribe={handleViewTribe} onJoinToggle={handleJoinToggle} onEditTribe={(tribe) => setEditingTribe(tribe)} onSharePost={handleSharePost} onLoadMore={loadMoreDiscover} hasMore={hasMoreDiscover} isFetchingMore={isFetchingMoreDiscover} />
//                         </PageContainer>
//                         <PageContainer name="Messages" activeName={activeNavItem}>
//                             <ChatPage currentUser={currentUser} allUsers={visibleUsers} chukUser={CHUK_AI_USER} initialTargetUser={chatTarget} onViewProfile={handleViewProfile} onSharePost={handleSharePost} />
//                         </PageContainer>
//                         <PageContainer name="Tribes" activeName={activeNavItem}>
//                             <TribesPage tribes={tribes} currentUser={currentUser} onJoinToggle={handleJoinToggle} onCreateTribe={handleCreateTribe} onViewTribe={handleViewTribe} onEditTribe={(tribe) => setEditingTribe(tribe)} />
//                         </PageContainer>
//                         <PageContainer name="Notifications" activeName={activeNavItem}>
//                             <NotificationsPage notifications={notifications} onViewProfile={handleViewProfile} onViewMessage={handleStartConversation} onViewPost={handleViewPost} />
//                         </PageContainer>
                        
//                         <div style={{ display: activeNavItem === 'TribeDetail' ? 'block' : 'none' }}>
//                            {viewedTribe ? <TribeDetailPage tribe={viewedTribe} currentUser={currentUser} userMap={userMap} isLoadingMessages={isFetchingTribeMessages} onSendMessage={handleSendTribeMessage} onDeleteMessage={handleDeleteTribeMessage} onDeleteTribe={handleDeleteTribe} onBack={() => setActiveNavItem('Tribes')} onViewProfile={handleViewProfile} onEditTribe={(tribe) => setEditingTribe(tribe)} onJoinToggle={handleJoinToggle} onOpenMembers={() => setMembersModalOpen(true)} /> : <div className="text-center p-8">Tribe not found. Go back to discover more tribes.</div>}
//                         </div>
//                         <div style={{ display: activeNavItem === 'Profile' ? 'block' : 'none' }}>
//                             {viewedUser && !(currentUser.blockedUsers || []).includes(viewedUser.id) && !(viewedUser.blockedUsers || []).includes(currentUser.id) ? <ProfilePage user={viewedUser} allUsers={users} visibleUsers={visibleUsers} allTribes={tribes} posts={userPosts} currentUser={currentUser} onLikePost={handleLikePost} onCommentPost={handleCommentPost} onDeletePost={handleDeletePost} onDeleteComment={handleDeleteComment} onViewProfile={handleViewProfile} onUpdateUser={handleUpdateUser} onAddPost={handleAddPost} isPosting={isCreatingPost} onToggleFollow={handleToggleFollow} onStartConversation={handleStartConversation} onNavigate={handleSelectItem} onSharePost={handleSharePost} onOpenStoryCreator={() => setIsCreatingStory(true)} myStories={myStories} onViewUserStories={handleViewUserStories} onToggleBlock={handleToggleBlock} /> : <div className="text-center p-8">User not found or is blocked.</div>}
//                         </div>
//                         <div style={{ display: activeNavItem === 'Settings' ? 'block' : 'none' }}>
//                             <SettingsPage currentUser={currentUser} onLogout={logout} onDeleteAccount={handleDeleteAccount} onToggleBlock={handleToggleBlock} allUsers={users} onBack={() => handleSelectItem('Profile')} />
//                         </div>
//                     </div>
//                 </main>
//             </React.Suspense>
...
//             {isMembersModalOpen && viewedTribe && <TribeMembersModal isOpen={isMembersModalOpen} onClose={() => setMembersModalOpen(false)} memberIds={viewedTribe.members} userMap={userMap} onViewProfile={handleViewProfile} />}
