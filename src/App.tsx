...
import { User, Post, Tribe, TribeMessage, Notification as NotificationType, Comment, Story, NavItem } from './types';
...
const PageContainer: React.FC<{ name: NavItem, activeName: NavItem, children: React.ReactNode, className?: string }> = React.memo(({ name, activeName, children, className }) => (
    <div style={{ display: name === activeName ? 'block' : 'none' }} className={className || 'h-full w-full'}>
        {children}
    </div>
));
...
const App: React.FC = () => {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            const populatedPosts = uniquePosts.map(p => populatePost(p)).filter(Boolean);
    ...
    const loadMoreFeed = useCallback(async () => {
        ...
        try {
            ...
            if (data.length > 0) {
                const newPosts = data.map((p: any) => populatePost(p)).filter(Boolean);
    ...
    return (
        <div className="bg-background min-h-screen text-primary overflow-hidden">
            ...
            <React.Suspense fallback={<div />}>
                ...
                <main ref={mainContentRef} className={mainClasses}>
                    <div className={contentContainerClass}>
                        ...
                        <PageContainer name="Messages" activeName={activeNavItem} className="h-full">
                        ...
                        <div style={{ display: activeNavItem === 'TribeDetail' ? 'block' : 'none' }} className="h-full">
                        ...
                        <div style={{ display: activeNavItem === 'Settings' ? 'block' : 'none' }} className="h-full">
