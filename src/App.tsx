...
const App: React.FC = () => {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            const [uRes, fRes, tRes, sRes, fsRes] = await Promise.all([
                ...
                api.fetchFeedPosts(1, 20).catch(() => ({ data: [] })),
    ...
    const handleViewProfile = async (user: User) => {
        ...
        try {
            ...
            // Fallback: filter local posts if API fails
    ...
    // Recovery Filtering: Use optional chaining to prevent crashes on partial data
                ...
                <div className={['Messages', 'TribeDetail', 'Settings', 'Notifications'].includes(activeNavItem) ? 'h-[calc(100vh-8rem)] md:h-[calc(100vh-4rem)]' : 'max-w-2xl mx-auto px-4 md:px-6 pt-6 pb-24 md:pb-8'}>
                    {activeNavItem === 'Home' && (
                        <>
                            ...
                            {visiblePosts.length === 0 && isDataLoaded && <div className="text-center py-20 text-secondary">Looking for posts... Check Discover if empty! 🐣</div>}
                    ...
                    {/* // FIX: Used cast to string to avoid TypeScript overlapping type error when checking for 'Chuk' route state. */}
                    {activeNavItem === 'Messages' && <ChatPage currentUser={currentUser} allUsers={users} chukUser={CHUK_AI_USER} initialTargetUser={(activeNavItem as string) === 'Chuk' ? CHUK_AI_USER : null} onViewProfile={handleViewProfile} onSharePost={()=>{}} />}
