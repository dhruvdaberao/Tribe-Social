...
// // @route   GET /api/tribes/my-tribes
...
//     try {
//         const tribe = await Tribe.findById(req.params.id);
//         if (!tribe) return res.status(404).json({ message: 'Not found' });
//         const isMember = tribe.members.includes(req.user.id);
//         if (isMember) {
//             if (tribe.owner.toString() === req.user.id.toString()) return res.status(400).json({ message: 'Owner cannot leave' });
//             tribe.members = tribe.members.filter(m => m.toString() !== req.user.id.toString());
//         } else {
//             tribe.members.push(req.user.id);
//         }
//         await tribe.save();
...
// @route   GET /api/tribes
// Recovery logic: Returns ALL tribes to ensure page visibility
router.get('/', protect, async (req, res) => {
    console.log(`[QUERY] Fetching all tribes for user: ${req.user.id}`);
    try {
        const tribes = await Tribe.find()
            .sort({ createdAt: -1 })
            .maxTimeMS(8000)
        ...
        console.log(`[RESULT] Found ${tribes.length} tribes`);
        res.json(tribes || []);
    } catch (error) {
        console.error("Tribe Recovery Error:", error.message);
        res.status(200).json([]);
    }
});
...
router.post('/', protect, async (req, res) => {
    ...
    try {
        ...
        const saved = await tribe.save();
        res.status(201).json(saved);
    } catch (error) {
        res.status(500).json({ message: 'Tribe creation failed' });
    }
});
...
router.put('/:id/join', protect, async (req, res) => {
    try {
        ...
        const isMember = tribe.members.some(m => m.toString() === req.user.id);
        if (isMember) {
            if (tribe.owner.toString() === req.user.id) return res.status(400).json({ message: 'Owner cannot leave' });
            tribe.members = tribe.members.filter(m => m.toString() !== req.user.id);
        } else {
            tribe.members.push(req.user.id);
        }
        await tribe.save();
        ...
        res.status(500).json({ message: 'Join operation failed' });
    }
});
