...
router.post('/send/:receiverId', protect, async (req, res) => {
    try {
        ...
        // Notify the receiver's socket for unread count, but don't create a persistent DB notification
        const recipientSocketId = req.onlineUsers.get(receiverId.toString());
        if (recipientSocketId) {
            req.io.to(recipientSocketId).emit('newMessage', responseMessage);
        }
