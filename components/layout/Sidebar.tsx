...
const Sidebar: React.FC<SidebarProps> = ({ activeItem, onSelectItem, currentUser, unreadMessageCount, unreadTribeCount, unreadNotificationCount, isChatOpen }) => {
  ...
  // Hidden on Tribe Chat (TribeDetail) AND Active DM Chat (Messages + isChatOpen) - ONLY ON MOBILE
  const shouldHideHeaderOnMobile = activeItem === 'TribeDetail' || (activeItem === 'Messages' && isChatOpen);

  return (
    <>
      {/* Top Header - Hidden on Mobile if Chat Open, Visible on Desktop */}
      <header className={`fixed top-0 left-0 right-0 h-16 bg-top-bar z-50 items-center justify-between px-4 md:px-6 ${shouldHideHeaderOnMobile ? 'hidden md:flex' : 'flex'}`}>
        ...
        <div className="flex items-center space-x-6 h-full">
          ...
          >
            {/* FIX: Reduced height to h-8/h-10 to fit in h-16 header properly and removed py-2 if needed */}
            <img
...
// Lucide Icons with refined stroke widths (1.75px base, 2.25px active)
const HomeIcon: React.FC<IconProps> = ({ isActive }) => <IconWrapper><Home size={24} strokeWidth={isActive ? 2.25 : 1.75} /></IconWrapper>;
const DiscoverIcon: React.FC<IconProps> = ({ isActive }) => <IconWrapper><Compass size={24} strokeWidth={isActive ? 2.25 : 1.75} /></IconWrapper>;
const MessagesIcon: React.FC<IconProps> = ({ isActive }) => <IconWrapper><MessageCircle size={24} strokeWidth={isActive ? 2.25 : 1.75} /></IconWrapper>;
