...
const PostCard: React.FC<PostCardProps> = (props) => {
  ...
  return (
    <>
    <div className={`bg-surface border-border rounded-2xl shadow-md overflow-hidden ${onModalClose ? '' : 'border mb-6'}`}>
      {/* Post Header */}
      <div className="flex items-center p-4">
        {onModalClose && (
            <button onClick={onModalClose} className="p-2 mr-2 -ml-2 text-primary rounded-full hover:bg-background">
        ...
        <div className="w-12 h-12 rounded-full cursor-pointer flex-shrink-0" onClick={() => onViewProfile(post.author)}>
             <UserAvatar user={post.author} className="w-full h-full" />
        </div>
        <div className="ml-4 min-w-0 flex-1">
          <p
            className="font-bold text-primary cursor-pointer hover:underline truncate"
            onClick={() => onViewProfile(post.author)}
          >
            {post.author.name}
          </p>
      ...
      {post.imageUrl && (
        <div className="bg-background w-full">
          {/* 
             Fix: Changed 'object-contain' to 'object-cover object-center'
             and enforced max-height to 500px to crop long images to the center.
          */}
          <img 
            src={post.imageUrl} 
            alt="Post content" 
