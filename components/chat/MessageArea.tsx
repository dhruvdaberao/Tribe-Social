...
export const MessageArea: React.FC<MessageAreaProps> = ({ conversation, messages, isLoading, currentUser, userMap, isSending, onSendMessage, onBack, onViewProfile }) => {
  ...
  useEffect(() => {
    if (!socket) return;
    const handleTyping = ({ userId }: { userId: string }) => { if (userId === otherParticipantId) setIsTyping(true); };
    const handleStopTyping = ({ userId }: { userId: string }) => { if (userId === otherParticipantId) setIsTyping(false); };
    socket.on('userTyping', handleTyping);
    socket.on('userStoppedTyping', handleStopTyping);
    return () => { socket.off('userTyping', handleTyping); socket.off('userStoppedTyping', handleStopTyping); };
  }, [socket, otherParticipantId]);

  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setInputText(e.target.value);
    if (socket && otherParticipantId) {
      if (!typingTimeoutRef.current) socket.emit('typing', { roomId: `dm-${[currentUser.id, otherParticipantId].sort().join('-')}`, userId: currentUser.id, userName: currentUser.name });
      if (typingTimeoutRef.current) clearTimeout(typingTimeoutRef.current);
      typingTimeoutRef.current = setTimeout(() => {
        socket.emit('stopTyping', { roomId: `dm-${[currentUser.id, otherParticipantId].sort().join('-')}`, userId: currentUser.id, userName: currentUser.name });
        typingTimeoutRef.current = null;
  ...
  if (!otherParticipant) return <div className="flex-1 flex items-center justify-center"><p className="text-secondary">User not found.</p></div>;

  return (
    <div className="flex flex-col h-full bg-background overflow-hidden">
      <div className="flex items-center p-3 border-b border-border bg-surface flex-shrink-0 z-10 h-16">
        <button onClick={onBack} className="md:hidden p-2 mr-2 text-primary"><BackIcon /></button>
        <div className="flex items-center cursor-pointer overflow-hidden" onClick={() => onViewProfile(otherParticipant)}>
            <UserAvatar user={otherParticipant} className="w-10 h-10 rounded-full mr-3 flex-shrink-0" isOnline={isOtherUserOnline} />
            <div className="min-w-0">
                <h2 className="text-lg font-bold text-primary leading-tight hover:underline truncate">{otherParticipant.name}</h2>
                 {isTyping ? <p className="text-sm text-accent italic">typing...</p> : <p className="text-sm text-secondary truncate">@{otherParticipant.username}</p>}
            </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {isLoading ? (
            <div className="w-full h-full flex items-center justify-center"><img src="/duckload.gif" className="w-16 h-16" /></div>
        ) : (
            <>
            {messages.map(message => {
                const isCurrentUser = message.senderId === currentUser.id;
                ...
                return (
                <div key={message.id} className={`flex items-end gap-2.5 ${isCurrentUser ? 'justify-end' : 'justify-start'}`}>
                    {!isCurrentUser && <div className="w-8 h-8 rounded-full flex-shrink-0 self-start"><UserAvatar user={sender || null} /></div>}
                    <div className={`flex flex-col w-full max-w-xs lg:max-w-md ${isCurrentUser ? 'items-end' : 'items-start'}`}>
                        <div className={`px-4 py-2.5 ${isCurrentUser ? 'bg-accent text-accent-text rounded-2xl rounded-tr-none' : 'bg-surface text-primary shadow-sm rounded-2xl rounded-tl-none'}`}>
                            ...
                            <div className="text-sm leading-relaxed">{sender?.id === 'chuk-ai' ? <MarkdownRenderer text={message.text} /> : <p className="whitespace-pre-wrap break-words">{message.text}</p>}</div>
                        </div>
                        <p className="text-xs text-secondary mt-1.5 px-1">{sentAt}</p>
                    </div>
                </div>
      ...
      <div className="p-4 bg-surface border-t border-border flex-shrink-0">
        <form onSubmit={handleSendMessage} className="flex items-center space-x-3">
          <input type="text" value={inputText} onChange={handleInputChange} placeholder="Type a message..." className="flex-1 bg-background border border-border rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-accent text-primary min-w-0" />
          <button type="submit" className="bg-accent text-accent-text rounded-lg w-12 h-12 flex-shrink-0 flex items-center justify-center hover:bg-accent-hover disabled:opacity-50" disabled={!inputText.trim() || isSending}>
            {isSending ? <div className="w-5 h-5 border-2 border-accent-text border-t-transparent rounded-full animate-spin"></div> : <SendIcon />}
          </button>
        </form>
      </div>
