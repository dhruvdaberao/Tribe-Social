...
export const MessageArea: React.FC<MessageAreaProps> = ({ conversation, messages, isLoading, currentUser, userMap, isSending, onSendMessage, onBack, onViewProfile }) => {
  ...
  }, [messages, isLoading, imageToSend]);
    ...
    <div className="flex flex-col h-full bg-background">
      ...
      <div className="p-4 bg-surface border-t border-border flex-shrink-0">
        ...
        <form onSubmit={handleSendMessage} className="flex items-center space-x-3">
          <button type="button" onClick={() => fileInputRef.current?.click()} className="p-2 text-secondary hover:text-accent rounded-full transition-colors"><CameraIcon /></button>
          ...
          />
          <button type="submit" className="bg-accent text-accent-text rounded-full w-11 h-11 flex-shrink-0 flex items-center justify-center hover:bg-accent-hover transition-colors disabled:opacity-50" disabled={(!inputText.trim() && !imageToSend) || isSending}>
