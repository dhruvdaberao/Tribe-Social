...
interface ConversationListProps {
  conversations: any[]; // Changed to any to accept populated otherUser
  isLoading: boolean;
  currentUser: User;
  chukUser: User;
    ...
    <div className="h-full flex flex-col bg-surface">
      ...
      <div className="overflow-y-auto flex-1">
          <ConversationItem
              ...
              isActive={activeConversationId === chukUser.id || activeConversationId === chukConversation.id}
        ...
        {isLoading ? (
            ...
            conversations.map(conv => {
              // Use the populated user data from backend if available, fallback to map
              const otherParticipant = conv.otherUser || userMap.get(conv.participants.find((p: any) => p.id !== currentUser.id)?.id || '');
              ...
              return (
                <ConversationItem
                  ...
                  onSelect={onSelectConversation}
                  unreadCount={unreadCounts[otherParticipant.id] || 0}
                />
              );
            })
        )}
