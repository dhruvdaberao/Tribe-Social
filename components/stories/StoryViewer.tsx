...
const StoryViewer: React.FC<StoryViewerProps> = ({ userStories, currentUser, allUsers, allTribes, onClose, onDelete, onLike, onSharePost }) => {
  ...
  return (
    <>
      ...
      {isShareModalOpen && <ShareModal post={{ author: user, content: currentStory.text || '', imageUrl: currentStory.imageUrl } as any} currentUser={currentUser} users={allUsers} tribes={allTribes} onClose={() => setIsShareModalOpen(false)} onSharePost={(_, dest) => handleShare(dest)} />}
    </>
  );
};
