        ...
        <div className="flex-1 overflow-y-auto p-4 bg-background">
          <div className="flex flex-col space-y-2">
            {localMessages.map(message => {
              ...
              const isCurrentUser = String(msgSenderId) === String(currentUser.id);
              
              // Get sender details: Prioritize the 'sender' object (populated by backend)
              // If not available, check userMap, otherwise Unknown.
              const sender = message.sender || userMap.get(msgSenderId || '') || { name: 'Anonymous', avatarUrl: null, id: 'unknown', username: 'unknown' };
