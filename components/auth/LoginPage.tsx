...
const LoginPage: React.FC = () => {
  ...
  const handleError = (err: any) => {
    if (axios.isAxiosError(err)) {
      // Check if backend returned a JSON error message
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'login') {
        ...
        if (!name || !username || !email || !password) throw new Error('Please fill in all fields.');
      ...
      if (err.message && !axios.isAxiosError(err)) setError(err.message);
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-surface p-8 rounded-2xl shadow-lg border border-border">
          ...
          <form onSubmit={handleSubmit}>
            ...
            <button type="submit" disabled={isLoading} className="w-full bg-accent text-accent-text font-bold p-3 rounded-lg hover:bg-accent-hover transition-all transform active:scale-[0.98] disabled:opacity-50">
