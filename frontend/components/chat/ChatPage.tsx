...
const ChatPage: React.FC<ChatPageProps> = ({ currentUser, allUsers, chukUser, initialTargetUser, onViewProfile, onSharePost, onConversationStateChange, onToggleBlock }) => {
  ...
  return (
    <div className="flex h-[100dvh] min-h-0 w-full flex-col overflow-hidden bg-surface md:flex-row">
      ...
      */}
      <div
        className={`min-h-0 w-full flex-1 flex-col bg-surface ${isMessageAreaVisible ? 'hidden md:flex md:w-[320px] md:flex-shrink-0 lg:w-[380px]' : 'flex'} md:border-r md:border-border`}
      >
        <ConversationList
