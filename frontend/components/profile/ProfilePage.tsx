                            ...
                            {isOwnProfile ? (
                                ...
                                <>
                                    ...
                                    <div className="relative">
                                        ...
                                        {optionsOpen && (
                                            <div className="absolute right-0 mt-2 w-48 bg-surface rounded-lg shadow-lg border border-border z-10 py-1">
                                                <ShareButton shareData={{ title: `Check out ${safeUser.name}'s profile on Tribe!`, text: `See what ${safeUser.name} (@${safeUser.username}) is up to.`, url: window.location.href }} className="w-full text-left px-4 py-2 text-primary hover:bg-background transition-colors flex items-center space-x-2" onShare={() => setOptionsOpen(false)}>
                                                    <ShareIcon className="h-4 w-4" /><span>Share Profile</span>
                                                </ShareButton>
                                                <button onClick={() => { onReportUser(safeUser.id); setOptionsOpen(false); }} className={`w-full text-left px-4 py-2 hover:bg-red-500/10 text-red-500 transition-colors flex items-center space-x-2`}>
                                                ...
                                                }} className={`w-full text-left px-4 py-2 hover:bg-background transition-colors flex items-center space-x-2 ${isBlocked ? 'text-secondary' : 'text-red-500'}`}>
