...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const initializeAuth = () => {
      try {
        ...
        console.warn("Auth initialization warning (storage might be restricted):", e);
      }
      setIsLoading(false);
    };
    initializeAuth();
  ...
  useEffect(() => {
    try {
      ...
      console.warn("Failed to persist user session to localStorage (Quota Exceeded):", e);
  ...
  const login = useCallback(async (email: string, password: string) => {
    ...
    try {
        localStorage.setItem('token', data.token);
    } catch (e) { console.warn("Could not save token"); }
    setCurrentUser(normalizeUser(data.user));
  }, []);
