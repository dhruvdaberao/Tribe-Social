...
const App: React.FC = () => {
    ...
    useEffect(() => {
        if (typeof document === 'undefined' || !viewportHeight) return;
    ...
    let containerClass = 'w-full max-w-2xl mx-auto px-4 md:px-6 pt-6 pb-24 md:pb-8'; // Default with bottom nav padding
    ...
    if (isFullHeightPage) {
        ...
        if (isChatPage) {
            containerClass = 'h-full min-h-0 w-full max-w-6xl mx-auto md:px-4 md:pb-4';
    ...
    return (
        <div className="bg-background h-[var(--vvh,100dvh)] min-h-[100dvh] text-primary flex flex-col">
            <Toaster />
            ...
            */}
            <main className={`flex-1 min-h-0 ${isChatPage ? 'overflow-hidden pt-16 md:pt-20' : 'overflow-y-auto overscroll-y-contain pt-16 pb-16 md:pb-0'}`}>
