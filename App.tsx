...
import { useAuth } from './contexts/AuthContext';
import { useSocket } from './contexts/SocketContext';
import { User, Post, Tribe, Notification as NotificationType, Comment, Story } from './types';
...
const App: React.FC = () => {
    ...
    useEffect(() => {
        ...
        // Tribe messages are now fully handled by TribeDetailPage to avoid conflicts and state duplication.
    ...
    const handleViewTribe = async (tribe: Tribe) => {
        try {
            clearUnreadTribe(tribe.id);
            // We just set the view, the detail page component will fetch messages
            setViewedTribe({ ...tribe, messages: [] });
            setActiveNavItem('TribeDetail');
        } catch (error) {
            console.error("Failed to set tribe view:", error);
