...
// // //     // Initialize from LocalStorage (Only for seen stories, everything else loads fresh)
// // //     useEffect(() => {
...
// // //         // Just set the view, let TribeDetailPage fetch the messages on mount
// // //         try {
// // //             clearUnreadTribe(tribe.id);
// // //             // Don't rely on global tribe.messages for history as it might be stale/empty
// // //             // We pass the tribe object, but the DetailPage will fetch history itself
// // //             setViewedTribe({ ...tribe, messages: [] });
// // //             setActiveNavItem('TribeDetail');
// // //         } catch (error) {
// // //             console.error("Error setting tribe view:", error);
// // //         }
// // //     };
...
// // // Use SessionStorage to prevent "Quota Exceeded" crashes while keeping the app snappy
// // // SessionStorage clears when the tab is closed, but persists during reloads/navigation
...
// //             if (cachedTribes) setTribes(cachedTribes);
...
// //                 const populatedPosts = feedPostsData.map((post: any) => populatePost(post, new Map())).filter(Boolean);
// //                 setPosts(populatedPosts as Post[]);
// //                 // Cache top 20 posts to session storage
// //                 saveToCache('posts', populatedPosts.slice(0, 20));
// //             }
...
const App: React.FC = () => {
    ...
    // Initialize - Restore seen stories
    useEffect(() => {
        const seen = localStorage.getItem('seenStoryAuthors');
        if (seen) setSeenStoryAuthors(new Set(JSON.parse(seen)));
    }, []);
