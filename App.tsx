...
//     try {
//       const [usersRes, feedRes, tribesRes, myStoriesRes, followingStoriesRes] =
//         await Promise.all([
//           api.fetchUsers(),
//           api.fetchFeedPosts(),
//           api.fetchTribes(),
...
//       setProfilePosts(prev => prev.map(p => (p.id === postId ? data : p)));
//     } catch {
...
//         <img src="/duckload.gif" className="w-16" />
//       </div>
//     );
//   }
...
//         onSelectItem={i => {
//           if (i === 'Profile') handleViewProfile(currentUser);
//           else if (i === 'Chuk') setActiveNavItem('Messages');
//           else setActiveNavItem(i);
//         }}
...
//         <div
//           className={
//             ['Messages', 'TribeDetail', 'Settings', 'Notifications'].includes(
//               activeNavItem
//             )
...
//               onDeleteAccount={() => {}}
//               onToggleBlock={() => {}}
//               onBack={() => setActiveNavItem('Profile')}
//             />
//           )}
//         </div>
//       </main>
...
const App: React.FC = () => {
  ...
  if (isAuthLoading) return <div className="h-screen flex items-center justify-center" />;
  if (!currentUser) return <LoginPage />;

  const visiblePosts = posts.filter(
    (p) => p.author && !currentUser.blockedUsers?.includes(p.author.id)
  );
  ...
  return (
    <div className="min-h-screen bg-background text-primary">
      ...
      <main className="pt-16">
        {activeNavItem === 'Home' && (
          <>
            <CreatePost currentUser={currentUser} onAddPost={fetchData} isPosting={false} />
            <StoryFeed
              myStories={myStories}
              followingUserStories={followingUserStories}
              currentUser={currentUser}
        ...
        {activeNavItem === 'TribeDetail' && viewedTribe?.id && (
          <TribeDetailPage
            tribe={viewedTribe}
            currentUser={currentUser}
            userMap={userMap}
            onSendMessage={(tribeId: string, text: string) => {
              api.sendTribeMessage(tribeId, { text });
            }}
            onDeleteMessage={(tribeId: string, messageId: string) => {
              api.deleteTribeMessage(tribeId, messageId);
            }}
            onJoinToggle={(tribeId: string) => {
              handleJoinTribe(tribeId);
            }}
            onBack={() => setActiveNavItem('Tribes')}
            onViewProfile={handleViewProfile}
            onEditTribe={() => {}}
