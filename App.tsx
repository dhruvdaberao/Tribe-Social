...
//     // HYDRATION: Priority load from localStorage for instant feel
//     const [users, setUsers] = useState<User[]>(() => JSON.parse(localStorage.getItem('cache_users') || '[]'));
//     const [posts, setPosts] = useState<Post[]>(() => JSON.parse(localStorage.getItem('cache_posts') || '[]'));
//     const [tribes, setTribes] = useState<Tribe[]>(() => JSON.parse(localStorage.getItem('cache_tribes') || '[]'));
//     const [myStories, setMyStories] = useState<Story[]>([]);
//     const [followingUserStories, setFollowingUserStories] = useState<{ user: User, stories: Story[] }[]>([]);
...
//         setIsFetching(true);
//         try {
//             const [usersRes, feedRes, tribesRes, myStoriesRes, followStoriesRes] = await Promise.all([
//                 api.fetchUsers().catch(() => ({ data: users })),
//                 api.fetchFeedPosts(1, 20).catch(() => ({ data: posts })),
//                 api.fetchTribes().catch(() => ({ data: tribes })),
//                 api.fetchMyStories().catch(() => ({ data: [] })),
...
//             localStorage.setItem('cache_users', JSON.stringify(usersRes.data));
//             localStorage.setItem('cache_posts', JSON.stringify(normalizedPosts));
//             localStorage.setItem('cache_tribes', JSON.stringify(tribesRes.data));
...
//     }, [isAuthLoading, currentUser, isDataLoaded, fetchData]);
...
//             const { data } = await api.toggleFollow(userId);
//             setCurrentUser(prev => prev ? { ...prev, following: data.following } : null);
//             // Refresh counts in users list
//             const usersRes = await api.fetchUsers();
//             setUsers(usersRes.data);
//         } catch (error) {
//             setCurrentUser(currentUser); // Rollback
...
//             };
//         }));
//         try { await api.likePost(postId); } catch (e) { fetchData(); }
//     };
...
//                             <CreatePost currentUser={currentUser} allUsers={users} myStories={myStories} onAddPost={handleAddPost} isPosting={false} onOpenStoryCreator={()=>{}} onViewUserStories={()=>{}} />
//                             <StoryFeed myStories={myStories} followingUserStories={followingUserStories} currentUser={currentUser} seenStoryAuthors={new Set()} onViewUserStories={()=>{}} />
//                             {visiblePosts.length === 0 && isDataLoaded && <div className="text-center py-20 text-secondary">Follow someone to see posts! 🐣</div>}
//                             <FeedPage posts={visiblePosts} currentUser={currentUser} allUsers={users} allTribes={tribes} onLikePost={handleLikePost} onCommentPost={()=>{}} onDeletePost={()=>{}} onDeleteComment={()=>{}} onViewProfile={(u)=> { setViewedUser(u); setActiveNavItem('Profile'); }} onSharePost={()=>{}} />
//                         </>
//                     )}
//                     {activeNavItem === 'Discover' && <DiscoverPage posts={visiblePosts} users={users} tribes={tribes} currentUser={currentUser} onLikePost={handleLikePost} onCommentPost={()=>{}} onDeletePost={()=>{}} onDeleteComment={()=>{}} onToggleFollow={handleToggleFollow} onViewProfile={(u)=> { setViewedUser(u); setActiveNavItem('Profile'); }} onViewTribe={(t)=>{ setViewedTribe(t); setActiveNavItem('TribeDetail'); }} onJoinToggle={()=>{}} onEditTribe={()=>{}} onSharePost={()=>{}} onLoadMore={()=>{}} />}
//                     {activeNavItem === 'Messages' && <ChatPage currentUser={currentUser} allUsers={users} chukUser={CHUK_AI_USER} initialTargetUser={null} onViewProfile={(u)=> { setViewedUser(u); setActiveNavItem('Profile'); }} onSharePost={()=>{}} />}
//                     {activeNavItem === 'Tribes' && <TribesPage tribes={tribes} currentUser={currentUser} onJoinToggle={()=>{}} onCreateTribe={()=>{}} onViewTribe={(t)=>{ setViewedTribe(t); setActiveNavItem('TribeDetail'); }} onEditTribe={()=>{}} />}
//                     {activeNavItem === 'TribeDetail' && viewedTribe && <TribeDetailPage tribe={viewedTribe} currentUser={currentUser} userMap={userMap} onSendMessage={()=>{}} onDeleteMessage={()=>{}} onDeleteTribe={()=>{}} onBack={()=>setActiveNavItem('Tribes')} onViewProfile={(u)=> { setViewedUser(u); setActiveNavItem('Profile'); }} onEditTribe={()=>{}} onJoinToggle={()=>{}} />}
//                     {activeNavItem === 'Notifications' && <NotificationsPage notifications={notifications} allTribes={tribes} onViewProfile={(u)=> { setViewedUser(u); setActiveNavItem('Profile'); }} onViewMessage={()=>{}} onViewPost={()=>{}} onViewTribe={()=>{}} onViewStory={()=>{}} />}
//                     {activeNavItem === 'Profile' && viewedUser && <ProfilePage user={viewedUser} allUsers={users} visibleUsers={users} allTribes={tribes} posts={visiblePosts.filter(p => p.author.id === viewedUser.id)} currentUser={currentUser} hasStory={false} onLikePost={handleLikePost} onCommentPost={()=>{}} onDeletePost={()=>{}} onDeleteComment={()=>{}} onViewProfile={(u)=>setViewedUser(u)} onUpdateUser={()=>{}} onAddPost={handleAddPost} isPosting={false} onToggleFollow={handleToggleFollow} onStartConversation={()=>{}} onNavigate={(i)=>setActiveNavItem(i)} onSharePost={()=>{}} onOpenStoryCreator={()=>{}} myStories={[]} onViewUserStories={()=>{}} />}
//                     {activeNavItem === 'Settings' && <SettingsPage currentUser={currentUser} allUsers={users} onLogout={logout} onDeleteAccount={()=>{}} onToggleBlock={()=>{}} onBack={() => setActiveNavItem('Profile')} />}
//                 </div>
//             </main>
//         </div>
...
const App: React.FC = () => {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            const [uRes, fRes, tRes, sRes, fsRes] = await Promise.all([
                api.fetchUsers().catch(() => ({ data: users })),
                api.fetchFeedPosts(1, 15).catch(() => ({ data: posts })),
                api.fetchMyTribes().catch(() => ({ data: tribes })),
                api.fetchMyStories().catch(() => ({ data: [] })),
                api.fetchFollowingStories().catch(() => ({ data: [] }))
            ]);
    ...
    }, [currentUser, normalizePost, users, posts, tribes]);
    ...
    return (
        <div className="bg-background min-h-screen text-primary overflow-hidden">
            <Toaster />
            <Sidebar activeItem={activeNavItem} onSelectItem={(i) => { setActiveNavItem(i); if(i==='Profile') handleViewProfile(currentUser); }} currentUser={currentUser} unreadMessageCount={unreadMessageCount} unreadTribeCount={unreadTribeCount} unreadNotificationCount={unreadNotificationCount} />
            <main className="pt-16 pb-16 md:pb-0">
                <div className={['Messages', 'TribeDetail', 'Settings', 'Notifications'].includes(activeNavItem) ? 'h-[calc(100vh-8rem)] md:h-[calc(100vh-4rem)]' : 'max-w-2xl mx-auto px-4 md:px-6 pt-6 pb-24 md:pb-8'}>
                    {activeNavItem === 'Home' && (
                        <>
                            ...
                            <StoryFeed myStories={myStories} followingUserStories={followingUserStories} currentUser={currentUser} seenStoryAuthors={new Set()} onViewUserStories={()=>{}} />
                            {visiblePosts.length === 0 && isDataLoaded && <div className="text-center py-20 text-secondary">Your feed is empty. Follow people or tribes! 🐣</div>}
                            <FeedPage posts={visiblePosts} currentUser={currentUser} allUsers={users} allTribes={tribes} onLikePost={()=>{}} onCommentPost={()=>{}} onDeletePost={()=>{}} onDeleteComment={()=>{}} onViewProfile={handleViewProfile} onSharePost={()=>{}} />
                        </>
                    )}
                    {activeNavItem === 'Discover' && <DiscoverPage posts={visiblePosts} users={users} tribes={tribes} currentUser={currentUser} onLikePost={()=>{}} onCommentPost={()=>{}} onDeletePost={()=>{}} onDeleteComment={()=>{}} onToggleFollow={handleToggleFollow} onViewProfile={handleViewProfile} onViewTribe={(t)=>{ setViewedTribe(t); setActiveNavItem('TribeDetail'); }} onJoinToggle={()=>{}} onEditTribe={()=>{}} onSharePost={()=>{}} onLoadMore={()=>{}} />}
                    ...
                    {activeNavItem === 'Profile' && viewedUser && (
                        <ProfilePage 
                            ...
                            onStartConversation={()=>{}} 
                            onNavigate={(i)=>setActiveNavItem(i)} 
                            onSharePost={()=>{}} 
                            onOpenStoryCreator={()=>{}} 
                            myStories={[]} 
                            onViewUserStories={()=>{}} 
