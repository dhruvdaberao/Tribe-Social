...
import { safeGetSession, safeSetSession } from './storage/safeStorage';
...
const App: React.FC = () => {
    ...
    // Initialize from SessionStorage using Safe Storage
    useEffect(() => {
        const loadCachedData = () => {
            const cachedPosts = safeGetSession<Post[]>(SESSION_KEY_PREFIX + 'posts');
            const cachedTribes = safeGetSession<Tribe[]>(SESSION_KEY_PREFIX + 'tribes');
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            const results = await Promise.allSettled([
                api.fetchUsers(),
                api.fetchFeedPosts(20), // Fetch first 20 items
                api.fetchTribes(),
                api.fetchNotifications(),
                api.fetchMyStories(),
                api.fetchFollowingStories(),
            ...
            if (feedPostsResult.status === 'fulfilled') {
                ...
                const populatedPosts = feedPostsData.map((post: any) => populatePost(post, new Map())).filter(Boolean);
                setPosts(populatedPosts as Post[]);
                // Cache top posts safely
                safeSetSession(SESSION_KEY_PREFIX + 'posts', populatedPosts.slice(0, 30)); 
            }
            ...
            if (tribesResult.status === 'fulfilled') {
                ...
                const populatedTribes = tribesData.map((tribe: any) => ({ ...tribe, messages: [] }));
                setTribes(populatedTribes);
                safeSetSession(SESSION_KEY_PREFIX + 'tribes', populatedTribes);
            }
    ...
    const fetchAllPostsForDiscover = useCallback(async () => {
      if (isAllPostsLoaded) return;
      try {
        const { data } = await api.fetchPosts(30); // Limit discover fetch
        const populated = data.map((post: any) => populatePost(post, userMap)).filter(Boolean);
