...
// The backend URL is now hardcoded here to prevent module loading failures
// that were causing a blank screen. This makes the API layer more robust.
// Fallback to localhost if environment variables fail, but favor production.
const API_URL = 'https://tribe-social-backend.onrender.com';
...
API.interceptors.request.use((req) => {
  ...
  if (!navigator.onLine) {
      ...
      // We could return a rejected promise here to stop the request, 
      // but let's let axios fail naturally or cached service worker handle it in PWA future
  }
  return req;
});
...
API.interceptors.response.use(
    ...
    (error) => {
        if (error.code === 'ERR_NETWORK' || error.code === 'ERR_NAME_NOT_RESOLVED') {
            console.error("Network Error - Check Backend URL or Internet Connection");
...
// Posts - Updated for Pagination
export const fetchPost = (id: string) => API.get(`/posts/${id}`);
export const fetchPosts = (limit?: number, lastCreatedAt?: string) => 
    API.get(`/posts`, { params: { limit, lastCreatedAt } });
export const fetchFeedPosts = (limit?: number, lastCreatedAt?: string) => 
    API.get(`/posts/feed`, { params: { limit, lastCreatedAt } });
